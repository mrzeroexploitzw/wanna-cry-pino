...
async function groupContext(jid) {
  ...
  try {
    ...
    syncGroupInfo(jid, { id:jid, subject:metadata.subject, description:metadata.desc, total_participant_count:metadata.participants?.length||0, participants:metadata.participants||[] });
    return { admins, info: metadata };
  } catch (error) { logger.warn({err:error}, 'GROUP_METADATA_FAILED'); return {admins:[],info:null}; }
}
...
export async function sendWhatsAppMessage(jid, content, options = {}) {
  ...
  try {
    ...
    return result;
  } catch(error) { releaseQueue(); recordTransportFailure(error); logger.error({err:error,jid},'OUTBOUND_SEND_FAILED'); throw error; }
}
...
async function sendResponse(jid, groupId, sender, response) {
  ...
  if(response.mediaOutput?.buffer){
    ...
    const content=media.type==='sticker'?{sticker:media.buffer}:media.type==='video'?{video:media.buffer,caption:media.caption||undefined,mimetype:media.mimeType}:{image:media.buffer,caption:media.caption||undefined,mimetype:media.mimeType};
...
async function processIncoming(message) {
  ...
  const {admins}=await groupContext(jid);
  ...
  if(groupId){
    ...
    if(aiMod.flagged){const count=warnUser({groupId,userId:from,reason:'ai-moderation',messageId:message.key.id||null});await sendWhatsAppMessage(groupId,{text:`╭─── 🛡️ THANXIE SAFETY ───╮\n\n@${name}\n⚠️ This content was flagged by the configured safety filter.\n\nWarning #${count}\n\n📜 GROUP RULES\n${groupRules(groupId)}\n╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯`,mentions:[message.key.participant||jid]},{groupId,sender:from});return;}
...
async function connect(){
  ...
  sock.ev.on('connection.update',async({connection,lastDisconnect})=>{
    ...
    if(connection==='close'){
      const code=new Boom(lastDisconnect?.error)?.output?.statusCode;sock=null;
      if(code===DisconnectReason.loggedOut||code===401){registered=false;logger.error('WHATSAPP_LOGGED_OUT_REPAIR_REQUIRED');return;}
      if(!started)return;
      clearTimeout(reconnectTimer);const delay=Math.min(120000,10000*Math.max(1,2**reconnectAttempts));reconnectAttempts+=1;
      reconnectTimer=setTimeout(()=>connect().catch(error=>logger.error({err:error},'RECONNECT_FAILED')),delay);
    }
  });
...
export async function requestPairingCode(phoneNumber){
  ...
  if(registered)throw new Error('ALREADY_PAIRED');
  if(Date.now()<pairingCooldownUntil)throw new Error('PAIRING_COOLDOWN');
  ...
  pairingInFlight=true;pairingNumber=normalized;
  try{const code=await sock.requestPairingCode(normalized);pairingCooldownUntil=Date.now()+30000;return{phone_number:normalized,pairing_code:code};}
  finally{pairingInFlight=false;}
}
